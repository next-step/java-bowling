...
- [x] 점수를 계산한다.
- [x] 각 프레임의 점수는 합산되는 방식으로 자세한 내용은 볼링 규칙을 참고한다.
- [x] 일반 투구는 점수가 프레임이 종료된 후 반영된다.
- [x] 스트라이크는 다음 2회 투구의 점수가 함께 반영된다.
- [x] 스페어는 다음 1회 투구의 점수가 함께 반영된다.
- [x] 10 프레임의 경우에는 추가 투구횟수가 없으므로 핀의 개수의 합이 점수가 된다.
- [x] Frame이 점수 계산의 주체가 되도록 개발한다.
...
- [x] 점수 계산을 담당할 `Score` 클래스를 정의한다.
    - [x] 점수 계산이 가능한 경우를 판단하는 `canCalculate()`를 정의한다.
    - [x] 점수를 반환하는 `calculate()`를 정의한다.
        - [x] 점수는 기본형으로 반환하는 것이 좋겠다.
        - [x] 아직 점수를 반환할 수 없는 경우에는 예외를 발생시킨다.
            - [x] 사용하는 측에서는 검증을 하도록 유도한다.
- [x] 남은 횟수를 담당하는 `LeftCount` 클래스를 정의한다.
...
- step4
    ...
    - [ ] 메서드의 깊이가 너무 길어지지 않도록 구현해보기
        - 뎁스 1단계를 지키는 형태에서는 개선이 불가능한 느낌이다.
