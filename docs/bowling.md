...
- [ ] 점수 계산을 담당할 `Score` 클래스를 정의한다.
    - [ ] 점수 계산이 가능한 경우를 판단하는 `canCalculate()`를 정의한다.
    - [ ] 점수를 반환하는 `calculate()`를 정의한다.
        - [ ] 점수는 기본형으로 반환하는 것이 좋겠다.
        - [ ] 아직 점수를 반환할 수 없는 경우에는 예외를 발생시킨다.
            - [ ] 사용하는 측에서는 검증을 하도록 유도한다.
