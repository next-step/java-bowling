...
- `State` 에서 점수 계산이 불가능한 상태는 예외를 던지기보다 `UnavailableScore` 를 사용하면 어떤가?
    - `UnavailableScore` 를 사용해도 `Frame` 의 `getScore()` 로직 상 점수 계산이 불가능하다면 예외를 던지기는 한다.
    - 로직을 따라가서 예외가 발생할 수 있는 메소드들을 검토해보고, 적용 시 예외 발생 가능성이 있는 부분이 줄어든다면 적용하자.
- `FrameCreator` 제거할 방법?
...
- `RollResultDto` 를 생성할 때, `Gutter` 문자열을 다른 곳에서 관리하는 것이 좋지 않을까?
    - 이미 다른 문자열들도 `StateExporter` 에서 관리하고 있으므로, `Gutter` 표기도 여기에 두도록 하자.
