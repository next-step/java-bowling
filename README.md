...
- 공을 던진 후 결과 값을 도메인에 `int` 로 전달하기 보다, `RollResult` 와 같은 도메인 타입으로 전달
- `State` 에서 점수 계산이 불가능한 상태는 예외를 던지기보다 `UnavailableScore` 를 사용하면 어떤가?
    - `UnavailableScore` 를 사용해보니 `Frame` 의 `getScore()` 로직 상 다음 프레임을 참조하기 때문에 마지막 프레임까지 도달하여 예외가 발생함.
    - 일단 변경하지 않고 그래도 두기로 한다.
- `FrameCreator` 제거할 방법?
    - 3단계 진행할 때, 다음 프레임을 현재 프레임에서 만들도록 하고 싶었으나 그러지 못했음.
    - 다음에 생성할 프레임을 현재 프레임이 직접 해야 한다면, `NormalFrame` 과 `FinalFrame` 중 어떤 것을 생성할 지 현재 프레임이 선택할 수 있어야 함.
    - 프레임 번호를 멤버로 사용한다거나 하는 방법을 고려했는데, 이러면 규칙 7에 위배된다.
    - 다른 클래스에 위임하면 `FrameCreator` 같은 클래스를 만들어야 한다.
    - 이런 이유로, `FrameCreator` 에서 모든 프레임을 초기화하도록 구현했다.
    - 제거할 방법은 아직도 생각나지 않아 일단 그대로 둔다.
- `RollResultDto` 를 생성할 때, `Gutter` 문자열을 다른 곳에서 관리하는 것이 좋지 않을까?
    - 규칙을 확인해보니, 두 번째 투구에서 핀을 쓰러트리지 못했을 때만 '-' 로 표기한다고 한다.
    - `StateExporter` 에 반영
...
- 테스트 `DisplayName` 변경
    - 보다 명확한 이름이 좋을 듯
- `Ready`, `Playing` 리팩토링
- `BowlingGame` 이 `Players` 에게 직접 `RollResult` 를 전달하여 투구 처리를 `Players` 가 처리하도록 리팩토링
