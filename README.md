...
- [ ] 예를 들면, 점수를 totalScore로 가지고 있지말고, 점수를 구하는 메소드를 지원해주는 방법도 있겠네요 🙂
- [ ] 위의 필드를 가지고있고, getPreviousTotalScore(), getPinchings()같은 중복되는 코드가 보이네요. 이 두 필드와 공통 메소드는 Frame이 가지고 있으면 어떨까요?
- [ ] 사용하지 않는 값으로 보입니다 ! 👀 (ScoreCalculateTest.java 169 line)
- [ ] 처음에 모든 프레임을 초기화시키는 것으로 보이는데요. 이렇게 초기화해주신 의도가 있을까요 ? 👀  볼링을 진행하면서 프레임을 생성해주는 방법은 어떨까요?
- [ ] 처음, 마지막, 현재 진행중인 프레임을 포인터처럼 가지고 있군요 🙂 자료구조를 활용해보는 방법도 있을 것 같아요 ! 
