...
class StateTest {
    ...
    public void isFinish_ReturnFinish(State state, boolean expected) {
        assertThat(state.isFinish()).isEqualTo(expected);
    }

    public static Stream<Arguments> source_isFinish_ReturnFinish() {
        return Stream.of(
                Arguments.of(State.NotFinish, false),
                Arguments.of(State.Finish, true),
                Arguments.of(State.Bonus, false));
    }
}
