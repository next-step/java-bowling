...
class NormalFrameTest {
    ...
    @Test
    void strike() {
        assertThat(new NormalFrame(10, 0).resultOfFrame()).isEqualTo(ScoreType.STRIKE.getSymbol());
    }

    @DisplayName("프레임의 두번재 투구에서 모든 핀(10개)을 쓰러트린 상태는 스페어 점수를 받는다.")
    @Test
    void spare() {
        Frame normalFrame = new NormalFrame(8, 0).next(2);
        assertThat(normalFrame.resultOfFrame()).isEqualTo(8 + ScoreType.SPARE.getSymbol());
    }
//
