...
class FrameTest {
    ...
    @Test
    void bowl() {
        Frame result = frame().bowl(FallenPin.of(2));

        Frame expected = new Frame(List.of(bowling().bowl(FallenPin.of(2))));
        assertThat(result).isEqualTo(expected);
    }
    ...
    void finished_whenBowlingFinished() {
        Frame frameWithTriesDoneBowling = frame().bowl(FallenPin.of(1));
        Frame frameWithStrikeBowling = Frame.init().bowl(FallenPin.of(10));
