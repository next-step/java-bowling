...
class FrameTest {
    ...
    @Test
    void bowl() {
        Frame result = frame().bowl(new Score(2));

        Frame expected = new Frame(List.of(bowling().bowl(new Score(2))));
        assertThat(result).isEqualTo(expected);
    }
    ...
    void finished_whenBowlingFinished() {
        Frame frameWithTriesDoneBowling = frame().bowl(new Score(1));
        Frame frameWithStrikeBowling = Frame.init().bowl(new Score(10));
