...
public class NormalFrameTest {
    ...
    void nextFrame() {
        Frame first = NormalFrame.bowling(round, firstCount).bowling(secondCount);

        assertThat(first.next(nextCount)).isNotEqualTo(first);
    }

    @Test
    @DisplayName("9번째 프레임 종료 후, LastFrame 생성")
    void finalNext() {
        Frame semiFinal = NormalFrame.bowling(round, firstCount).bowling(secondCount);
