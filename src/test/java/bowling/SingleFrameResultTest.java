...
import bowling.domain.SingleFrameResult;
...
public class SingleFrameResultTest {
    ...
    void When_pointSymbols_Then_ReturnPointSymbols() {
        PointSymbols pointSymbols = new PointSymbols(PointSymbol.STRIKE);
        SingleFrameResult singleFrameResult = new SingleFrameResult(pointSymbols);

        assertThat(singleFrameResult.pointSymbols()).isEqualTo(pointSymbols);
    }

    @Test
    @DisplayName("초기 점수는 미확정")
    void Given_Initial_When_Score_Then_NotDetermined() {
        assertThat(new SingleFrameResult().score()).isEqualTo(Score.createNotDetermined());
    }
}
