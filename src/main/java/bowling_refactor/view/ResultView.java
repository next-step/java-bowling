...
import bowling_refactor.domain.*;
...
public class ResultView {
    ...
    public static void printScoreBoardPlayers(Players players) {
        System.out.println(HEADER);
        for (int i = 0; i < players.size(); i++) {
            printScoreBoardPlayer(players.getPlayer(i));
        }
    }

    public static void printScoreBoardPlayer(Player player) {
        printStatus(player);
        printScore(player);
    }
