...
public class ResultView2 {
    ...
    public static void print(GameResults gameResults) {
        printLineSeparator();
        printResult(gameResults, gameResults.getFirstResult(), gameResults.getSecondResult());
    }

    private static void printResult(GameResults gameResults, GameResult gameResult1, GameResult gameResult2) {
        IntStream.rangeClosed(FRAME_ID_FIRST, FRAME_ID_FINAL)
                .forEach(frameId -> printFrameResultSoFar(gameResults, gameResult1, gameResult2, frameId));
    }

    private static void printFrameResultSoFar(GameResults gameResults, GameResult gameResult1, GameResult gameResult2, int frameId) {
        if (frameId != FRAME_ID_FINAL) {
            printFrameResult(FIRST, gameResults, frameId);
            printFrameResult(SECOND, gameResults, frameId);
        }

        if (frameId == FRAME_ID_FINAL) {
            printFrameResult(FIRST, gameResults, frameId);
            printFrameResult(SECOND, gameResults, frameId);
            printFrameResult(THIRD, gameResults, frameId);
            printFrameResult(FOURTH, gameResults, frameId);
        }
    }

    private static void printFrameResult(Ordinal ordinal, GameResults gameResults, int frameId) {
        for(int i=0; i<gameResults.getSize(); i++) {
            GameResult gameResult = gameResults.getResultByIndex(i);

            if (gameResult.getFrameByFrameId(frameId).containsOrdinal(ordinal)) {
                print(gameResult.getName() + "'s turn");
                print(" : ");
                println(gameResult.getFrameByFrameId(frameId).getPointAtOrdinal(ordinal));
                printPlayInformation();

                printForOnePlayer(ordinal, gameResult, gameResult.getFrameByFrameId(frameId));
                printLineSeparator();

                if (frameId == FRAME_ID_FIRST && FIRST.equals(ordinal)) {
                    printName(gameResult.getPlayerName().getName());
                }

                if (frameId == FRAME_ID_FIRST && SECOND.equals(ordinal)) {
                    printForOnePlayer(FIRST, gameResult, gameResult.getFrameByFrameId(frameId));
                }

                if (frameId != FRAME_ID_FIRST && FIRST.equals(ordinal)) {
                    printForOnePlayer(SECOND, gameResult, gameResult.getFrameByFrameId(frameId + OFFSET));
                }
                ...
                if (frameId == FRAME_ID_FINAL
                        ...
                        && gameResult.getFrameByFrameId(frameId).containsOrdinal(FOURTH)) {
                    printForOnePlayer(FIRST, gameResult, gameResult.getFrameByFrameId(frameId));
                }

                if (frameId == FRAME_ID_FINAL
                        && FOURTH.equals(ordinal)) {
                    printForOnePlayer(THIRD, gameResult, gameResult.getFrameByFrameId(frameId));
                }
            ...
            if (!gameResult.getFrameByFrameId(frameId).containsOrdinal(ordinal)) {
                ...
                printForOnePlayer(SECOND, gameResult, gameResult.getFrameByFrameId(frameId));
                printThreeLineSeparators();
            }
        }
    }
