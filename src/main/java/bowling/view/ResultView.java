...
public class ResultView {
    ...
    private static String makeResultString(PitchHistory pitchHistory){
        if(PitchResultEnum.STRIKE == pitchHistory.getResult()){
            return STRIKE_STRING;
        }
        if(PitchResultEnum.SPARE == pitchHistory.getResult()){
            return SPARE_STRING;
        }
        if(PitchResultEnum.MISS == pitchHistory.getResult()){
            return pitchHistory.getDownPin() + MISS_STRING;
        }
        if(PitchResultEnum.GUTTER == pitchHistory.getResult()){
            return GUTTER_STRING;
        }
        if(PitchResultEnum.NONE == pitchHistory.getResult()){
